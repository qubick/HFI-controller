...
THREE.CSG = {
	toCSG: function ( three_model, offset, rotation ) {
		...
		if ( three_model instanceof THREE.Mesh ) {
			// console.log("three_model: ", three_model);
			if (three_model.geometry instanceof THREE.BufferGeometry) //to load stl
				geometry = new THREE.Geometry().fromBufferGeometry( three_model );
			else
			...
			if (three_model instanceof THREE.BufferGeometry) //to load stl
				geometry = new THREE.Geometry().fromBufferGeometry( three_model );
			else
		...
		for ( i = 0; i < geometry.faces.length; i++ ) {
			if ( geometry.faces[i] instanceof THREE.Face3 ) {
				...
				vertices.push( new CSG.Vertex( rotation_matrix.multiplyVector3( geometry.vertices[geometry.faces[i].a]), [ geometry.faces[i].normal.x, geometry.faces[i].normal.y, geometry.faces[i].normal.z ] ) );
				vertices.push( new CSG.Vertex( rotation_matrix.multiplyVector3( geometry.vertices[geometry.faces[i].b]), [ geometry.faces[i].normal.x, geometry.faces[i].normal.y, geometry.faces[i].normal.z ] ) );
				vertices.push( new CSG.Vertex( rotation_matrix.multiplyVector3( geometry.vertices[geometry.faces[i].c]), [ geometry.faces[i].normal.x, geometry.faces[i].normal.y, geometry.faces[i].normal.z ] ) );
